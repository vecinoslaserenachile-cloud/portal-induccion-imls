...
import { 
  ...
  Printer, RefreshCw, XCircle, User, FileText
} from 'lucide-react';
...
const RadioPlayer = () => (
  <div className="fixed bottom-0 w-full bg-slate-900/95 backdrop-blur-md text-white p-3 flex items-center justify-between z-50 border-t border-slate-700 h-16 shadow-2xl print:hidden">
    ...
    <div className="hidden md:block text-[10px] text-slate-500 uppercase tracking-widest font-bold">Inducción 2026</div>
...
export default function App() {
  ...
  // --- ARREGLO DEL SCROLL (Lógica Anti-Bloqueo) ---
  const checkProgress = () => {
    const el = scrollRef.current;
    if (el) {
      // 1. Si ya bajó todo (con margen de 100px para pantallas grandes)
      const isAtBottom = el.scrollHeight - el.scrollTop - el.clientHeight < 100;
      // 2. O SI EL TEXTO ES CORTO y no necesita scroll (Arreglo del bug)
      const isShortContent = el.scrollHeight <= el.clientHeight + 50;
      
      if (isAtBottom || isShortContent) setCanAdvance(true);
    }
  };
  ...
  useEffect(() => {
    ...
    if ([0, 1, 9, 10].includes(step)) {
      ...
      setCanAdvance(false);
      // Revisión automática inicial por si el texto es corto
    ...
    <div className="h-screen w-full flex flex-col lg:flex-row bg-white text-slate-900 overflow-hidden pb-16 print:hidden">
      ...
      <div className="w-full lg:w-1/2 flex flex-col h-full pt-12 relative z-10 bg-white shadow-2xl">
        ...
        <div ref={scrollRef} onScroll={checkProgress} className="flex-1 overflow-y-auto px-8 lg:px-12 py-6 space-y-6 text-lg text-slate-600 leading-relaxed text-justify">
          {content}
          {!canAdvance && (
            <div className="h-24 flex items-center justify-center opacity-60 animate-pulse text-red-500 font-bold text-sm">
              <ChevronDown className="animate-bounce mr-2"/> Sigue leyendo para avanzar
            </div>
          )}
        ...
        <div className="px-8 lg:px-12 py-6 border-t border-slate-100 bg-slate-50 flex items-center justify-between shrink-0 h-24">
           <div className="text-xs font-bold uppercase tracking-widest">
             {canAdvance ? 
               ...
               <span className="text-slate-400 flex items-center gap-2"><ChevronDown size={16}/> Lee todo el contenido</span>
             }
           </div>
  ...
  if (step === 0) return (
    <div className="h-screen w-full flex flex-col lg:flex-row bg-slate-900 overflow-hidden relative print:hidden">
      <div className="absolute inset-0">
        <img src="/img/portada.jpg" onError={(e) => e.currentTarget.src='https://images.unsplash.com/photo-1517048676732-d65bc937f952?q=80&w=2070'} className="w-full h-full object-cover opacity-40" alt="Fondo" />
      ...
      <div className="relative z-10 w-full flex flex-col items-center justify-center text-center p-8 overflow-y-auto">
        ...
        <div className="bg-white/10 backdrop-blur-md p-8 rounded-3xl border border-white/20 max-w-lg w-full space-y-4 shadow-2xl">
          ...
          {/* SEGUNDA FILA: Teléfono y Cargo (RESTITUIDOS CON ÍCONOS SEGUROS) */}
          <div className="flex gap-4">
             <div className="relative w-1/2">
                {/* Usamos FileText como icono seguro para contacto */}
                <FileText className="absolute top-4 left-4 text-slate-400" size={18}/>
             ...
             <div className="relative w-1/2">
                {/* Usamos User como icono seguro para cargo */}
                <User className="absolute top-4 left-4 text-slate-400" size={18}/>
          ...
          <div className="relative">
            <select className="w-full p-4 rounded-xl bg-white/90 font-bold text-slate-900" onChange={e => setUserData({...userData, dept: e.target.value})}>
              <option value="">Selecciona Dirección / Depto...</option>
  ...
  switch (step) {
    case 1: return <ChapterLayout title="Bienvenida Oficial" subtitle="Mensaje de la Alcaldesa" 
      ...
      content={
        <>
           ...
           <p>Es un honor recibirte en la Ilustre Municipalidad de La Serena. Asumes hoy un compromiso vital con la segunda ciudad más antigua de Chile.</p>
           <p>Esta inducción ha sido diseñada para entregarte las herramientas fundamentales. Nuestra administración se caracteriza por la cercanía, la transparencia y la innovación.</p>
           <p>Te invitamos a ver el mensaje de nuestra Alcaldesa Daniela Norambuena, quien te contará los desafíos que enfrentamos juntos en este periodo 2024-2028. ¡Bienvenido al equipo!</p>
        </>
      } 
    />;
    ...
    case 2: return <ChapterLayout title="Concejo Municipal" subtitle="Periodo 2024 - 2028" 
      ...
      content={
        <>
          <p>El <strong>Concejo Municipal</strong> es el órgano colegiado encargado de hacer efectiva la participación de la comunidad local. Su función es normativa y fiscalizadora.</p>
          <p>Está compuesto por la Alcaldesa y <strong>10 concejales</strong>, elegidos democráticamente. Ellos representan las distintas sensibilidades de La Serena.</p>
          <p>Es fundamental que conozcas a nuestras autoridades. Las sesiones de Concejo son públicas y se transmiten para garantizar la transparencia.</p>
    ...
    case 3: return <ChapterLayout title="Nuestra Estructura" subtitle="Visión General" 
      ...
      content={
        <>
          <p>El municipio se organiza bajo una estructura jerárquica clara:</p>
          <p><strong>Nivel Directivo:</strong> Alcaldía, Secretaría Municipal y Administración Municipal definen la estrategia.</p>
          <p><strong>Nivel Operativo:</strong> DIDECO, DOM y Tránsito ejecutan los programas en terreno.</p>
          <p><strong>Nivel de Apoyo:</strong> Jurídica, Control y Gestión de Personas dan soporte transversal.</p>
        </>
      } 
    />;

    case 4: return <ChapterLayout title="Organigrama Detallado" subtitle="Mapa Completo" 
      ...
      content={
        <>
          ...
          <p>Identifica tu ubicación exacta: ¿De qué Dirección dependes? La comunicación formal debe respetar siempre los conductos regulares.</p>
          <p>Observa las unidades de Control y Jurídica: ellas velan por la legalidad de nuestros actos.</p>
        </>
      } 
    />;
      ...
      visual={
        <div className="relative w-full h-full flex items-center justify-center bg-slate-50">
           <div className="grid grid-cols-2 gap-8 p-12">
             <div className="bg-white p-6 rounded-2xl shadow-lg border-l-4 border-blue-500"><h4 className="font-bold text-blue-600">CIUDADANÍA</h4><p className="text-sm">Juntas de Vecinos, Usuarios.</p></div>
             ...
             <div className="bg-white p-6 rounded-2xl shadow-lg border-l-4 border-green-500"><h4 className="font-bold text-green-600">PRIVADOS</h4><p className="text-sm">Proveedores, Gremios.</p></div>
             <div className="bg-white p-6 rounded-2xl shadow-lg border-l-4 border-purple-500"><h4 className="font-bold text-purple-600">INTERNO</h4><p className="text-sm">Funcionarios, Concejo.</p></div>
           </div>
        </div>
      }
      content={
        <>
          <p>Interactuamos con múltiples actores:</p>
          ...
          <p><strong>3. Gremios:</strong> Actores claves para la gobernanza local.</p>
        </>
      } 
    />;

    case 6: return <ChapterLayout title="Ley Karin" subtitle="Espacios Seguros" visual={<div className="flex items-center justify-center h-full bg-pink-50"><Heart size={200} className="text-pink-500 animate-pulse" /></div>} 
      content={
        <>
          <p>La <strong>Ley N° 21.643 (Ley Karin)</strong> previene y sanciona el acoso laboral, sexual y la violencia en el trabajo.</p>
          <p>Ya no se exige "reiteración" para denunciar. Un solo acto grave basta. También sanciona la violencia de terceros hacia funcionarios.</p>
          <p>El Municipio tiene tolerancia cero y cuenta con protocolos de denuncia confidencial.</p>
        </>
      } 
    />;

    case 7: return <ChapterLayout title="Remuneraciones" subtitle="Tu Vínculo" visual={<div className="flex items-center justify-center h-full bg-green-50"><DollarSign size={200} className="text-green-600" /></div>} 
      content={
        <>
          <p>Estamentos principales:</p>
          <p><strong>1. Planta:</strong> Cargos permanentes por ley.</p>
          <p><strong>2. Contrata:</strong> Cargos transitorios anuales.</p>
          <p><strong>3. Honorarios:</strong> Prestadores de servicios específicos.</p>
          <p><strong>Pago:</strong> Planta y Contrata reciben su sueldo el <strong>penúltimo día hábil del mes</strong>.</p>
        </>
      } 
    />;

    case 8: return <ChapterLayout title="Seguridad" subtitle="Normas" visual={<div className="flex items-center justify-center h-full bg-yellow-50"><Shield size={200} className="text-yellow-600" /></div>} 
      content={
        <>
          <p><strong>Reglamento Interno:</strong> Regula el control horario, credencial, permisos (6 días administrativos) y obligaciones.</p>
          <p><strong>Seguridad:</strong> Conoce las vías de evacuación (Cota 30). Ante accidentes laborales, informa INMEDIATAMENTE para activar el seguro ACHS.</p>
        </>
      } 
    />;
        ...
        <div className="max-w-2xl w-full">
          {!quizFinished ? (
            ...
            <div className="bg-white text-slate-900 p-12 rounded-[3rem] text-center shadow-2xl">
              ...
              <p className="text-xl text-slate-500 mb-8">{score >= 7 ? "¡Aprobado!" : "Debes repasar."}</p>
              {score >= 7 ? (
      ...
      <div className="min-h-screen bg-slate-800 flex items-center justify-center p-4">
        <div className="bg-white p-12 max-w-5xl w-full aspect-[1.4/1] relative shadow-2xl flex flex-col items-center justify-between text-center border-[20px] border-double border-slate-200 print:w-full print:h-screen print:border-none print:shadow-none print:absolute print:top-0 print:left-0 print:m-0">
          ...
          <div className="w-full flex justify-between items-start mb-8">
             <img src="/img/escudo.png" onError={(e) => e.currentTarget.style.display='none'} className="h-24 object-contain" alt="Escudo"/>
             <img src="/img/innovacion.png" onError={(e) => e.currentTarget.style.display='none'} className="h-24 object-contain" alt="Sello"/>
          </div>

          <div className="flex-1 flex flex-col items-center justify-center">
             <h1 className="text-6xl font-serif font-black text-slate-900 mb-2 uppercase tracking-wide">CERTIFICADO</h1>
             <p className="text-xl text-slate-500 italic mb-8">De Aprobación de Inducción</p>
          ...
          <div className="flex justify-between w-full px-10 mt-12 items-end">
            <div className="text-center">
              ...
              <p className="text-xs font-bold uppercase text-slate-600">Gestión de Personas</p>
            </div>
            ...
            <div className="text-center">
               ...
               <div className="w-48 h-0.5 bg-slate-400 mb-2 mx-auto"></div>
               <p className="text-xs font-bold uppercase text-slate-600">Alcaldesa</p>
            </div>
          </div>
        </div>
