...
const ProgressBar = ({ progress }) => (
  <div className="fixed top-0 w-full h-1.5 bg-slate-800 z-50">
    <div 
      className="h-full bg-red-600 transition-all duration-1000 ease-out shadow-[0_0_15px_rgba(220,38,38,0.8)]" 
...
function App() {
  ...
  // Lógica "Anti-Bloqueo": Valida si hay poco texto o si ya bajaste
  const checkProgress = () => {
    const el = scrollRef.current;
    if (el) {
      const isShort = el.scrollHeight <= el.clientHeight + 50; // Margen de seguridad
  ...
  useEffect(() => {
    // Pasos automáticos (Videos, Bienvenida, Fin)
    if (step === 0 || step === 1 || step >= 14) {
      setCanAdvance(true);
    } else {
      setCanAdvance(false);
      setTimeout(checkProgress, 800); // Esperar a que cargue el texto
    }
  ...
  const ChapterLayout = ({ title, subtitle, content, image, youtubeId = null }) => (
    <div className="h-screen w-full flex flex-col lg:flex-row bg-white text-slate-900 overflow-hidden pb-16">
      ...
      <div className="w-full lg:w-1/2 flex flex-col h-full pt-12 relative z-10 bg-white">
        ...
        {/* ÁREA DE SCROLL CON DETECCIÓN */}
        <div 
          ref={scrollRef} 
          onScroll={checkProgress} 
          className="flex-1 overflow-y-auto px-8 lg:px-16 py-6 space-y-6 text-lg text-slate-600 leading-relaxed"
        >
          {content}
        ...
        <div className="px-8 lg:px-16 py-6 border-t border-slate-100 bg-slate-50 flex items-center justify-between shrink-0 h-24">
           <div className="text-xs font-bold uppercase tracking-widest">
             {canAdvance ? 
               <span className="text-green-600 flex items-center gap-2 animate-pulse"><CheckCircle size={16}/> Lectura Completada</span> : 
           ...
           <button 
             ...
             className="bg-red-600 text-white px-8 py-3 rounded-full font-bold shadow-lg hover:bg-slate-900 disabled:bg-slate-200 disabled:text-slate-400 transition-all flex items-center gap-2"
           >
        ...
        {youtubeId ? (
          ...
          <>
            <img 
              ...
              onLoad={checkProgress} // ESTO ES CLAVE PARA QUE NO SE PEGUE
            />
    ...
    <div className="h-screen w-full flex flex-col lg:flex-row bg-slate-900 overflow-hidden">
      ...
      <div className="flex-1 p-12 bg-white flex items-center justify-center">
        <div className="max-w-md w-full space-y-6">
          ...
          <input className="w-full p-4 border border-slate-300 rounded-xl bg-slate-50 text-slate-900" placeholder="Nombres" onChange={e => setUserData({...userData, nombres: e.target.value})} />
  ...
  switch (step) {
    case 1: return <ChapterLayout title="Bienvenida" subtitle="Daniela Norambuena" youtubeId="yA_86g9Bq_U" content={<p>¡Hola! Soy Serenito...</p>} />;
    ...
    case 3: return <ChapterLayout title="Valores" subtitle="La Lucha Municipalidad" image="ESTRATEGIA_IMLS.png" content={
      <>
        ...
        <p>Este texto es largo para probar el scroll. Sigue bajando...</p>
        <p>Más contenido sobre la estrategia Asset Light...</p>
        <p>¡Listo! Al llegar aquí, el botón rojo se activará.</p>
      </>
    } />;
